...
Deno.serve(async (request) => {
  ...
  const sourceText = (body.text ?? "").trim().slice(0, MAX_TEXT_CHARACTERS);
  if (!sourceText) return jsonResponse({ error: "missing_text" }, 400);
  ...
  const model = Deno.env.get("OPENAI_TEXT_TRANSLATION_MODEL") ?? "gpt-4.1-mini";
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model,
      instructions:
        `Translate the user's text into ${outputLanguage} (${outputLanguageCode}). ` +
  ...
  if (!response.ok) {
    return jsonResponse({ error: "openai_text_translation_failed", detail: data }, response.status);
  }

  const translatedText = extractOutputText(data);
  if (!translatedText) return jsonResponse({ error: "empty_translation", detail: data }, 502);
